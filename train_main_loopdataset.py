...
########### Siqi Ye, 2023-08, Rad Onc., Stanford ###################

#### This code applies the MLINR algorithm to the entire dataset reported in the paper
#### citation: [Ye, S., Shen, L., Islam, M. T., & Xing, L. (2023). Super-resolution biomedical imaging via reference-free statistical implicit neural representation. Physics in Medicine & Biology, 68(20), 205020.]
...
loopdata_path = os.path.join(opts.output_path + "/outputs",config['data']+'_loopdata')
test_rmse_collect = []
test_psnr_collect = []
test_ssim_collect = []
...
## ## for CT data
## for idata in range (14, 78):  # 92 slices
##     print('Load image: {}'.format(config['img_path']))
##     img_path = os.path.join(config['img_path']+str(idata)+'.npz')
##     data_name = config['data'] + '_' + str(idata)

## for ultrasound data
for idata in range (1, 85):  # 85 images
    print('Load image: {}'.format(config['img_path']))
    img_path = os.path.join(config['img_path']+str(idata)+'.tif')
    data_name = config['data'] + '_' + str(idata)


## ## for microscopy data
## img_list = ['02','03','04','05','06','07','09','10','11','12','13','14','15','16']
## num_data = len(img_list)
## print('num_data = ', num_data)
## for idata in range(len(img_list)):
##     print('Load image: {}'.format(config['img_path']))
##     img_i = img_list[idata]
##     print('img_i', img_i)
##     img_path = os.path.join(config['img_path']+img_i+'_I_t1_SRRF.tif')
##     data_name = config['data'] + '_' + img_i

########################### start main training ######################3
    # Setup output folder
    output_folder = os.path.splitext(os.path.basename(opts.config))[0]
    if opts.pretrain: 
        output_subfolder = data_name + '_pretrain'
    else:
        output_subfolder = data_name 
    if config['ker_type'] == 'Avg':
        model_name = os.path.join(loopdata_path,output_folder, output_subfolder + '/SRscale{}_numLRs{}_blur{}_kerSz{}_{}_{}_{}_{}_{}_{}_lr{:.2g}_StepLR{}_{}_wd{}_encoder_{}_embedSz{}_scale{}' \
        ...
        model_name = os.path.join(output_folder, output_subfolder + '/SRscale{}_numLRs{}_{}_{}_{}_{}_{}_{}_{}_lr{:.2g}_StepLR{}_{}_wd{}_encoder_{}_embedSz{}_scale{}' \
    ...
    shifts = shifts * config['downscale']/2
    ...
    data_loader = get_data_loader_HR(data_name,config['isTIFF'], img_path, config['HR_dim'],0, 0, train=True, batch_size=config['batch_size'])
    ...
    for it, (grid, img_HR, HR_max) in enumerate(data_loader):
       ...
       # save_image_3d(img_HR.cpu().data, slice_idx, os.path.join(image_directory, "HR_org.png"))
        print('HR_max_val = ', HR_max) 
        print('dataloader HR shape:', img_HR.shape)
        print('grid shape:', grid.shape)
        ### generate or load  LR obervations #########
        if config['is_gen_LR'] == 1: 
        ...
        else: # load real LR images 
            LR_1 = np.load(config['LR_file1'])['data']
            imsize = LR_1.shape
            if not(imsize[0] == imsize[1]):
                zerp_padding = np.zeros([imsize[0], np.int((imsize[0] - imsize[1])/2)])
                LR_1 =  np.concatenate([zerp_padding, LR_1, zerp_padding], axis=1)
            LR_1 = LR_1 / HR_max
            LR_1 = torch.tensor(LR_1, dtype=torch.float32)[:,:,None]
    
            LR_2 = np.load(config['LR_file2'])['data']
            imsize = LR_2.shape
            if not(imsize[0] == imsize[1]):
                zerp_padding = np.zeros([imsize[0], np.int((imsize[0] - imsize[1])/2)])
                LR_2 =  np.concatenate([zerp_padding, LR_2, zerp_padding], axis=1)
            LR_2 = LR_2 / HR_max
            LR_2 = torch.tensor(LR_2, dtype=torch.float32)[:,:,None]
            
            imgs_LR = torch.cat([LR_1, LR_2], dim=0)
        ...
        for iterations in range(max_iter):
            ...
            if iterations == 0 or (iterations + 1) % config['val_iter'] == 0:
                ...
                plt.ylabel('SSIM')
                plt.grid(True)
                fig3.savefig(os.path.join(log_path + "/validate_SSIM.png"),dpi=fig3.dpi)
    ...
    test_psnr_collect.append(test_psnr)
    test_ssim_collect.append(test_ssim)
    torch.save(test_psnr_collect, os.path.join(loopdata_path + "/MLINR_test_psnr_collect_" + str(opts.downscale) +".pt"))
    torch.save(test_ssim_collect, os.path.join(loopdata_path + "/MLINR_test_ssim_collect_" + str(opts.downscale) +".pt"))
